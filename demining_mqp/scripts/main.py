            ...
            for index in currentAdjacentSpaces:
                ...
                for archivedSpacesIndex in archivedSpaces:
                    if archivedSpacesIndex["x"] == index["x"] and archivedSpacesIndex["y"] == index["y"]:
                        inArchive = 1
                if inArchive == 0:
                    if index["x"]-halfSizeOfRobot<0 or index["y"]-halfSizeOfRobot<0 or index["x"]+halfSizeOfRobot>len(robotGrid[0]) - 1 or index["y"]+halfSizeOfRobot>len(robotGrid[0]) - 1 or robotGrid[index["x"]][index["y"]] == 4 or robotGrid[index["x"]][index["y"]] == 3 or robotGrid[index["x"]][index["y"]] == 0 or robotGrid[index["x"]][index["y"]] == 6:
                        index["dist"] = -1
                    elif robotGrid[index["x"]][index["y"]] == 2:
                        match = 0
            ...
            if robotGrid[detectX][detectY] == 2:
                for i in range(-halfSizeOfRobot + detectX,halfSizeOfRobot+detectX+1):
                    for j in range(-halfSizeOfRobot + detectY, halfSizeOfRobot + detectY + 1):
                        ...
                        elif robotGrid[i][j] == 0 or robotGrid[i][j] == 3 or robotGrid[i][j] == 4:
                            robotGrid[detectX][detectY] = 6
            if robotGrid[detectX][detectY] == 3 or robotGrid[detectX][detectY] == 4:
                for i in range(-halfSizeOfRobot + detectX,halfSizeOfRobot+detectX+1):
                    for j in range(-halfSizeOfRobot + detectY, halfSizeOfRobot + detectY + 1):
                        if i < 0 or j < 0 or j > len(robotGrid[0])-1 or i > len(robotGrid)-1:
                            print("do nothing")
...
def defineRobotGrid():
    x = 5
    y = 4
    robotGrid = [[5] * y for _ in range(x)]
    return robotGrid

#def defineRobotGrid2(/*win*/):
def defineRobotGrid2():
    x = 30
    y = 30
    graphicGrid = []
    for i in range(0, x):
        graphicGrid.append([])
        #for j in range(0, y):
            #graphicGrid[i].append(Rectangle(Point(50+10*i-5, 500-10*j-5), Point(50+10*i+5, 500-10*j+5)))
    ...
    for i in range (0,x):
        for j in range(0,y):
            ...
            if i<3 and j < 21:
                robotGrid[i][j] = 5
            robotGrid[8][12] = 0
    return robotGrid, graphicGrid

#def printRobotGrid(robotX,robotY, /*win,*/ graphicGrid, halfSizeOfRobot):
def printRobotGrid(robotX,robotY, graphicGrid, halfSizeOfRobot):
            ...
            #    #print(str(robotGrid[j][i])+" ", end="")
             #   if robotGrid[j][i] == 2:
              #      graphicGrid[j][i].setFill('green')
               ...
               # if robotGrid[j][i] == 4:
              #      graphicGrid[j][i].setFill('yellow')
             #   if robotGrid[j][i] == 5:
...
def scanRobotGrid(robotGrid, unreachableSquare):
...
def scanDirection(direction, robotX, robotY, robotGrid,halfSizeOfRobot):
    mine = False
    for index in range(-halfSizeOfRobot,halfSizeOfRobot+1):
        if direction == 1:
            detectX = robotX+index
...
def batteryService(battery, x, y , chargeLoc, maxBattery):
    rospy.wait_for_service('battery_life')
    try:
        batteryService = rospy.ServiceProxy('battery_life', batteryLifeService)
        resp1 = batteryService(battery, x ,y, chargeLoc, maxBattery)
	battery = resp1.newBatteryLife
...
if __name__=="__main__":
    rospy.wait_for_service('battery_life')
    #rospy.init_node('batteryLifeTalker', anonymous=True)
    halfSizeOfRobot = 1
    ...
    robotGrid, graphicGrid = defineRobotGrid2()
    ...
    for i in range(-halfSizeOfRobot + robotX, halfSizeOfRobot + robotX + 1):
        for j in range(-halfSizeOfRobot + robotY, halfSizeOfRobot + robotY + 1):
            if i<0 or j<0 or j>len(robotGrid[0])-1 or i > len(robotGrid)-1:
                ...
                robotGrid[i][j]=2
    #printRobotGrid(robotX, robotY, /*win,*/ graphicGrid, halfSizeOfRobot)
    printRobotGrid(robotX, robotY, graphicGrid, halfSizeOfRobot)
    #input("Press Enter to continue...")
    while hasUnexploredTerritory and hasValidMove:
        if (robotX != halfSizeOfRobot or robotY!=halfSizeOfRobot):
            batteryRoute, batteryTargetX, batteryTargetY, batteryDist = determineBatteryPath(robotX, robotY, halfSizeOfRobot, halfSizeOfRobot,robotGrid, halfSizeOfRobot)
	    ...
	    printRobotGrid(robotX, robotY, graphicGrid, halfSizeOfRobot)
		    ...
		    hasUnexploredTerritory = scanRobotGrid(robotGrid, unreachableSquares)
                    break
        if route!=[]:
            print("route")
            direction = route[len(route)-1]
            print("direction", direction)
            newMine, robotGrid = scanDirection(direction, robotX, robotY, robotGrid,halfSizeOfRobot)
            #printRobotGrid(robotX, robotY, /*win,*/ graphicGrid, halfSizeOfRobot)
	    printRobotGrid(robotX, robotY, graphicGrid, halfSizeOfRobot)
            #("Press Enter to continue...")
            print("new mine", newMine, route, direction, robotGrid[targetX][targetY])
            if robotGrid[targetX][targetY]==3 or robotGrid[targetX][targetY]==4 or robotGrid[targetX][targetY]==6:
                route = []
            else:
                if newMine:
	...
	printRobotGrid(robotX,robotY, graphicGrid, halfSizeOfRobot)
        ...
        batteryLife = batteryService(batteryLife, robotX, robotY, halfSizeOfRobot, maxBattery)
        #input("Press Enter to continue...")
    #win.close()
